...
```python
xgb_model = xgb.XGBClassifier(max_depth=4, n_estimators=120, learning_rate=0.01)
```
### Step 6. Train the model with the given parameters Early Stopping Rounds=10, Evaluation Metric='error'.
```python
eval_set = [(X_train, y_train), (X_test, y_test)]
xgb_model.fit(X_train, y_train, early_stopping_rounds=10, eval_metric="error", eval_set=eval_set, verbose=True)
```
